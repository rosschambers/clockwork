...
# pi provider config: points the worker's pi sessions at an OpenAI-compatible model
# endpoint. Written to the container user's HOME where pi reads it. The tracked
# docker/pi-models.json is a TEMPLATE with a placeholder host ("your-model-host") —
# copy it to docker/pi-models.local.json (untracked) with your real endpoint before
# building. The local file wins when present.
RUN mkdir -p /root/.pi/agent \
	&& if [ -f docker/pi-models.local.json ]; then \
		cp docker/pi-models.local.json /root/.pi/agent/models.json; \
	else \
		cp docker/pi-models.json /root/.pi/agent/models.json; \
	fi
