...
export class DbStore {
	...
	createProject(input: CreateProjectInput): DbProject {
		...
		this.run(`
			INSERT INTO projects (id, name, description, github_repo, branch)
			VALUES (?, ?, ?, ?, ?)
		`, id, input.name, input.description, input.githubRepo ?? null, input.branch ?? null)
	...
	createCard(input: CreateCardInput): DbCard {
		const id = crypto.randomUUID()
		this.run(`
			INSERT INTO cards (id, project_id, column_id, title, body, position, retry_count)
			VALUES (?, ?, ?, ?, ?, ?, 0)
		`,
			id,
			input.projectId,
