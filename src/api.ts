...
export interface ServerConfig {
	...
	notifyToken?: string
	gitToken?: string
	// The real-merge capability behind POST /sync/reconcile. Optional so dev
	// servers (and most tests) run without one; when absent the endpoint returns
	// 400 "sync reconcile unavailable". Kept structural (not the RepoWorkspace
	// class) so tests can inject a fake and index.ts passes the real instance.
	repoWorkspace?: {
		reconcileSync(
...
function parseUpdateProject(body: any): { ok: boolean; data?: any; error?: string } {
	...
	if (body.github_upstream !== undefined) {
		updates.githubUpstream = body.github_upstream
	}
...
function parseCardRequest(body: any): { ok: boolean; data?: any; error?: string } {
	...
	return { ok: true, data: {
		...
		dependsOn: typeof body.depends_on === "string" ? body.depends_on : null,
	}}
}
...
async function handleCardsChain(request: Request, dbStore: DbStore, projectId: string, broker: WsBroker): Promise<Response> {
	const project = dbStore.getProjectById(projectId)
	if (!project) {
		return errorResponse(404, "project not found")
	}
	...
	const nodes = buildChain(body.items)
	const created: DbCard[] = []

	for (const node of nodes) {
		const dependsOn = node.dependsOnIndex === null ? null : created[node.dependsOnIndex]!.id
		const pos = nextPositionInColumn(dbStore, projectId, body.column_id)
		const card = dbStore.createCard({
			projectId,
			columnId: body.column_id,
			title: node.title,
		...
		const withScenario = node.scenario === null ? card : dbStore.updateCard(card.id, { scenario: node.scenario })
		created.push(withScenario)
		broker.broadcast({ type: "card.created", cardId: card.id, projectId, columnId: card.columnId, timestamp: Date.now() } as WSMessage)
	}
...
async function handleRequeue(request: Request, dbStore: DbStore, id: string, broker: WsBroker): Promise<Response> {
	...
	const fromColumn = existing.columnId
	const pos = nextPositionInColumn(dbStore, existing.projectId, body.column_id)
	dbStore.moveCard(id, body.column_id, pos, false)
	dbStore.updateCard(id, { retryCount: 0, stageRetries: {} })
	dbStore.unclaimCard(id)
	const updated = dbStore.getCardById(id)!
	broker.broadcast({ type: "card.moved", cardId: id, projectId: existing.projectId, fromColumn, toColumn: body.column_id, timestamp: Date.now() } as WSMessage)
	return jsonResponse(200, updated)
}

async function handleRescope(request: Request, dbStore: DbStore, id: string, broker: WsBroker): Promise<Response> {
	const existing = dbStore.getCardById(id)
	if (!existing) {
		return errorResponse(404, "card not found")
	}
	...
	if (Array.isArray(body.targets)) {
		updates.targets = body.targets.map((t: any) => String(t))
	}

	const updated = dbStore.updateCard(id, updates)
	broker.broadcast({ type: "card.updated", cardId: id, projectId: existing.projectId, timestamp: Date.now() } as WSMessage)
	return jsonResponse(200, updated)
}

async function handleSetScenario(request: Request, dbStore: DbStore, id: string, broker: WsBroker): Promise<Response> {
	const existing = dbStore.getCardById(id)
	if (!existing) {
		return errorResponse(404, "card not found")
	}
	...
	const updated = dbStore.updateCard(id, { scenario })
	broker.broadcast({ type: "card.updated", cardId: id, projectId: existing.projectId, timestamp: Date.now() } as WSMessage)
	return jsonResponse(200, updated)
}

async function handleSetDependsOn(request: Request, dbStore: DbStore, id: string, broker: WsBroker): Promise<Response> {
	const existing = dbStore.getCardById(id)
	if (!existing) {
		return errorResponse(404, "card not found")
	}
	...
	const dependsOn = typeof body.depends_on === "string" ? body.depends_on : null
	const updated = dbStore.updateCard(id, { dependsOn })
	broker.broadcast({ type: "card.updated", cardId: id, projectId: existing.projectId, timestamp: Date.now() } as WSMessage)
	return jsonResponse(200, updated)
}

async function handleAbandon(request: Request, dbStore: DbStore, id: string, broker: WsBroker): Promise<Response> {
	const existing = dbStore.getCardById(id)
	if (!existing) {
		return errorResponse(404, "card not found")
	}

	// Locate the Needs-Human park column the same way the worker does (worker.ts:728).
	const needsHumanColumn = dbStore
		.getColumnsByProject(existing.projectId)
		.find((c) => c.name.toLowerCase().includes("human"))
	if (!needsHumanColumn) {
		return errorResponse(400, "no needs-human column exists")
	}

	const fromColumn = existing.columnId
	const pos = nextPositionInColumn(dbStore, existing.projectId, needsHumanColumn.id)
	dbStore.moveCard(id, needsHumanColumn.id, pos, false)
	dbStore.unclaimCard(id)
	const updated = dbStore.getCardById(id)!
	broker.broadcast({ type: "card.moved", cardId: id, projectId: existing.projectId, fromColumn, toColumn: needsHumanColumn.id, timestamp: Date.now() } as WSMessage)
	return jsonResponse(200, updated)
}

function route(request: Request, dbStore: DbStore, requiredToken: string | null, broker: WsBroker, maxRetries: number, notifyUrl: string | undefined, notifyToken: string | undefined, repoWorkspace: ServerConfig["repoWorkspace"]): Response | Promise<Response> {
	...
	if (method === "POST" && projectMatch.match && projectMatch.rest === "/cards/chain") {
		return handleCardsChain(request, dbStore, projectMatch.param!, broker)
	}
...
export function startServer(config: ServerConfig): ServerHandle {
	...
	const bunServer = Bun.serve({
		...
		fetch(request: Request): Response | Promise<Response> | undefined {
			...
			return route(request, dbStore, requiredToken, broker, config.maxRetries ?? 3, config.notifyUrl, config.notifyToken, config.repoWorkspace)
		},
