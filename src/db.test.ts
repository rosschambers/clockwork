...
describe("DbStore — cards", () => {
	...
	it("new card has empty stageRetries map", () => {
		const card = store.createCard({
			projectId,
			columnId,
			title: "Card",
		...
		expect(card.stageRetries).toEqual({})
	})

	it("persists and reloads a per-stage retry map via updateCard", () => {
		const card = store.createCard({
			projectId,
			columnId,
			title: "Card",
		...
		store.updateCard(card.id, { stageRetries: { [columnId]: 2 } })
		const reloaded = store.getCardById(card.id)
		expect(reloaded!.stageRetries).toEqual({ [columnId]: 2 })
	})
...
describe("DbStore — releaseClaimsByWorker (restart recovery)", () => {
	it("frees only the cards claimed by the given worker", () => {
		const { store, path } = createTempDb()
		const project = store.createProject({ name: "P", description: "", githubRepo: null, branch: null })
		const column = store.createColumn({ projectId: project.id, name: "Impl", prompt: "", skills: [], model: null, position: 0 })
		const a = store.createCard({ projectId: project.id, columnId: column.id, title: "A", body: "", position: 0 })
		const b = store.createCard({ projectId: project.id, columnId: column.id, title: "B", body: "", position: 1 })
		const c = store.createCard({ projectId: project.id, columnId: column.id, title: "C", body: "", position: 2 })

		store.claimCardIfFree(a.id, "main")
		store.claimCardIfFree(b.id, "main")
		store.claimCardIfFree(c.id, "other-worker")

		const released = store.releaseClaimsByWorker("main")

		expect(released).toBe(2)
		expect(store.getCardById(a.id)!.claimState).toBeNull()
		expect(store.getCardById(b.id)!.claimState).toBeNull()
		// A different worker's claim is untouched.
		expect(store.getCardById(c.id)!.claimState).toBe("claimed")
