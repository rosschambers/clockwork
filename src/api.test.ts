...
describe("REST API", () => {
	...
	describe("director actions", () => {
		...
		it("POST /api/cards/:id/reset-retry returns 404 for unknown card", async () => {
			const res = await fetch(`${baseUrl}/api/cards/nope/reset-retry`, { method: "POST" })
			expect(res.status).toBe(404)
		})
		...
		it("POST /api/cards/:id/requeue returns 404 for unknown card", async () => {
			const res = await fetch(`${baseUrl}/api/cards/nope/requeue`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ column_id: "x" }),
			})
			expect(res.status).toBe(404)
		})
		...
		it("POST /api/cards/:id/rescope returns 404 for unknown card", async () => {
			const res = await fetch(`${baseUrl}/api/cards/nope/rescope`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
		...
		it("POST /api/cards/:id/scenario stores a per-card scenario", async () => {
			const { cardId } = await seedCard()
			const res = await fetch(`${baseUrl}/api/cards/${cardId}/scenario`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ scenario: "godot" }),
			})
			expect(res.status).toBe(200)
			const data: any = await res.json()
			expect(data.scenario).toBe("godot")
		})

		it("POST /api/cards/:id/scenario returns 404 for unknown card", async () => {
			const res = await fetch(`${baseUrl}/api/cards/nope/scenario`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ scenario: "godot" }),
			})
		...
		it("POST /api/cards/:id/depends-on sets depends_on", async () => {
			const { cardId } = await seedCard()
			const res = await fetch(`${baseUrl}/api/cards/${cardId}/depends-on`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ depends_on: "other-card-id" }),
			})
			expect(res.status).toBe(200)
			const data: any = await res.json()
			expect(data.dependsOn).toBe("other-card-id")
		})

		it("POST /api/cards/:id/depends-on accepts null to clear the dependency", async () => {
			const { cardId } = await seedCard()
			await fetch(`${baseUrl}/api/cards/${cardId}/depends-on`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ depends_on: "x" }),
			})
			const res = await fetch(`${baseUrl}/api/cards/${cardId}/depends-on`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ depends_on: null }),
			})
			expect(res.status).toBe(200)
			const data: any = await res.json()
			expect(data.dependsOn).toBeNull()
		})

		it("POST /api/cards/:id/depends-on returns 404 for unknown card", async () => {
			const res = await fetch(`${baseUrl}/api/cards/nope/depends-on`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ depends_on: "x" }),
			})
		...
		it("POST /api/cards/:id/abandon moves to needs-human + clears claim", async () => {
			const { projectId, cardId } = await seedCard()
			const human = db.createColumn({ projectId, name: "Needs-Human", prompt: "", skills: [], model: null, position: 9 })
			db.claimCard(cardId, "w")
			const res = await fetch(`${baseUrl}/api/cards/${cardId}/abandon`, { method: "POST" })
			expect(res.status).toBe(200)
			const data: any = await res.json()
			expect(data.columnId).toBe(human.id)
			expect(data.claimState).toBeNull()
		})

		it("POST /api/cards/:id/abandon returns 404 for unknown card", async () => {
			const res = await fetch(`${baseUrl}/api/cards/nope/abandon`, { method: "POST" })
			expect(res.status).toBe(404)
		})

		it("POST /api/cards/:id/abandon returns 400 when no needs-human column exists", async () => {
			const { cardId } = await seedCard()
			const res = await fetch(`${baseUrl}/api/cards/${cardId}/abandon`, { method: "POST" })
			expect(res.status).toBe(400)
		})
	...
	describe("card chain", () => {
		it("POST /api/projects/:id/cards/chain creates a dependency-ordered chain", async () => {
			const project = db.createProject({ name: "P", description: "", githubRepo: null, branch: null })
			const col = db.createColumn({ projectId: project.id, name: "Impl", prompt: "", skills: [], model: null, position: 0 })
			const res = await fetch(`${baseUrl}/api/projects/${project.id}/cards/chain`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({
					column_id: col.id,
			...
			expect(created[0].dependsOn).toBeNull()
			expect(created[1].dependsOn).toBe(created[0].id)
			expect(created[2].dependsOn).toBe(created[1].id)
			expect(created[1].scenario).toBe("two.yaml")
		})

		it("returns 404 for an unknown project", async () => {
			const res = await fetch(`${baseUrl}/api/projects/nope/cards/chain`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ column_id: "c", items: [{ title: "x" }] }),
			})
			expect(res.status).toBe(404)
		})

		it("returns 400 for missing column_id or empty items", async () => {
			const project = db.createProject({ name: "P2", description: "", githubRepo: null, branch: null })
			const noCol = await fetch(`${baseUrl}/api/projects/${project.id}/cards/chain`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ items: [{ title: "x" }] }),
			})
			expect(noCol.status).toBe(400)
			const col = db.createColumn({ projectId: project.id, name: "Impl", prompt: "", skills: [], model: null, position: 0 })
			const noItems = await fetch(`${baseUrl}/api/projects/${project.id}/cards/chain`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ column_id: col.id, items: [] }),
			})
...
describe("POST /api/projects/:id/sync/reconcile", () => {
	...
	beforeEach(() => {
		...
		db = result.store
		dbPath = result.path
		handle = startServer({ dbStore: db, port: 0 })
		baseUrl = `http://127.0.0.1:${handle.port}`
	})
