...
import { WsBroker, wsHandler } from "./ws.ts"
...
describe("WebSocket events", () => {
	...
	it("broadcasts card.parked with classification and suggested actions", async () => {
		// api.ts does not expose the broker on ServerHandle (Task 9 owns api.ts),
		// so stand up a minimal Bun ws server with our own broker to drive the
		// real broadcast + subscription path directly — same open/subscribe/assert
		// pattern as the API-backed tests above.
		const broker = new WsBroker()
		const brokerServer = Bun.serve({
			port: 0,
			fetch(request: Request): Response | undefined {
				if (brokerServer.upgrade(request, { data: {} })) {
					return undefined
				}
		...
		const brokerWsUrl = `ws://127.0.0.1:${brokerServer.port}/ws`
		...
		// Subscribe to the project so this client is eligible to receive the event.
		...
		const parkedEvent = received.find((e) => e.type === "card.parked")
		expect(parkedEvent).toBeDefined()
		expect(parkedEvent.cardId).toBe("c1")
		expect(parkedEvent.projectId).toBe("p1")
		expect(parkedEvent.reason).toBe("deliverable-missing")
		expect(parkedEvent.suggestedActions).toEqual(["requeueCard", "reScopeCard", "abandonCard"])
		expect(parkedEvent.timestamp).toBeDefined()
	...
	it("delivers sync.rejected to subscribers of its project only", async () => {
		// Same direct-broker-over-a-real-socket pattern as the card.parked test above:
		// stand up a minimal Bun ws server so the broadcast + subscription filtering
		...
		const brokerWsUrl = `ws://127.0.0.1:${brokerServer.port}/ws`
		const wsMine = new WebSocket(brokerWsUrl)
		const wsOther = new WebSocket(brokerWsUrl)
		...
		await new Promise<void>((resolve) => {
			...
			wsMine.onopen = markReady
			wsOther.onopen = markReady
		})

		const received: any[] = []
		wsMine.onmessage = (event) => {
			received.push(typeof event.data === "string"
				? JSON.parse(event.data)
				: JSON.parse(new TextDecoder().decode(event.data)))
		}
		...
		wsOther.onmessage = (event) => {
			receivedOther.push(typeof event.data === "string"
				? JSON.parse(event.data)
				: JSON.parse(new TextDecoder().decode(event.data)))
		}

		wsMine.send(JSON.stringify({ type: "subscribe", projectId: "proj-A" }))
		wsOther.send(JSON.stringify({ type: "subscribe", projectId: "proj-B" }))
		await new Promise((r) => setTimeout(r, 20))
