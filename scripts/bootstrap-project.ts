...
export function pipelineColumns(dense: string): Array<{
	name: string
	prompt: string
	position: number
	model: string | null
...
Emit "pass" only when you have positively confirmed the change works and nothing regressed; "fail" with the exact reproduction + observed vs expected if it does not; "blocked" if you cannot run the verification (say why).`,
		...
		{
			name: "Deploy",
			prompt: `You are a DEPLOYER. Only act on cards that passed review AND QA. Verify the branch is ready to ship: tests green and reviewed, from ground truth (read the actual output) — never a "done" reply. Trigger any build-and-ship steps defined in PROJECT.md and confirm they landed (the pipeline succeeded, the artifact exists). DO NOT merge the card branch into main yourself — clockwork merges the card branch to main automatically when the card enters the terminal Done column. Your job is to prove the branch is deploy-ready and run project-specific ship steps, not to perform the git merge. Emit "pass" once the branch is verified ready (and any ship steps succeeded); "fail" if not (include the error).`,
			position: 600,
			model: DENSE,
			skills: [],
		},
...
async function bootstrap(name: string, githubRepo: string): Promise<void> {
	...
	// DENSE = the local dense model via the arbiter low port (clockwork's default provider).
	const DENSE = "local-dense-27b.gguf"
	const columns = pipelineColumns(DENSE)
